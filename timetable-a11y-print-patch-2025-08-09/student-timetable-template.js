...
/* === A11y Enhancements for Controls (2025-08-09) === */
function enhanceA11yForControls() {
  try {
    const printBtn = document.getElementById('printBtn');
    if (printBtn) {
      if (!printBtn.hasAttribute('aria-label')) printBtn.setAttribute('aria-label', '시간표 인쇄');
      if (!printBtn.getAttribute('title')) printBtn.setAttribute('title', '인쇄 (Ctrl+P)');
    }

    const bookmarkBtn = document.getElementById('bookmarkBtn');
    if (bookmarkBtn) {
      if (!bookmarkBtn.hasAttribute('aria-label')) bookmarkBtn.setAttribute('aria-label', '즐겨찾기에 추가');
      if (!bookmarkBtn.hasAttribute('aria-pressed')) bookmarkBtn.setAttribute('aria-pressed', 'false');

      // Sync aria-pressed on click without interfering with existing logic
      bookmarkBtn.addEventListener('click', function () {
        const current = bookmarkBtn.getAttribute('aria-pressed');
        const next = (current === 'true') ? 'false' : 'true';
        bookmarkBtn.setAttribute('aria-pressed', next);
      }, { passive: true });
    }

    // Hide decorative title icons/emojis from AT
    document.querySelectorAll('img.title-icon, .title-emoji').forEach(function(el) {
      el.setAttribute('alt', '');
      el.setAttribute('aria-hidden', 'true');
    });
  } catch (e) {
    // no-op; a11y enhancement should never break main flow
    console.warn('[a11y] enhanceA11yForControls failed:', e);
  }
}

// Ensure it runs after DOM is ready and after any template render
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', enhanceA11yForControls, { once: true });
} else {
  // If DOM already ready, still run
  setTimeout(enhanceA11yForControls, 0);
}
/* === End A11y Enhancements for Controls === */
